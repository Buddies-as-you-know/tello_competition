...
def main():
    global imgnum
    def rc(a=0,b=0,c=0,d=0):
        if a < 0:
            tello.move_left(abs(a))
        elif a > 0:
            tello.move_right(abs(a))

        if b < 0:
            tello.move_back(abs(b))
        elif b > 0:
            tello.move_forward(abs(b))

        if c < 0:
            tello.move_down(abs(c))
        elif c > 0:
            tello.move_up(abs(c))

        if d < 0:
            tello.rotate_counter_clockwise(abs(d))
        elif d > 0:
            tello.rotate_clockwise(abs(d))  
    ...
    try:
        ...
        while True:
            ...
            ret, bi_small = cv2.threshold(gray_small, 0, 255, cv2.THRESH_OTSU)
            # (4) ウィンドウに表示
            cv2.imshow('OpenCV Window', bi_small)    # ウィンドウに表示するイメージを変えれば色々表示できる
            ...
            if key == 27:                   # key が27(ESC)だったらwhileループを脱出，プログラム終了
                ...
                cv2.imwrite('/Users/babakeita/Desktop/tello'+str(imgnum)+".jpg", small_image)
                imgnum+=1
            ...
            cu_time = time.time()
            if cu_time - pr_time > interval:
                cv2.imwrite('/Users/babakeita/Desktop/res3/tello'+str(imgnum)+".jpg", bi_small)
                pr_time = cu_time
                imgnum+=1
